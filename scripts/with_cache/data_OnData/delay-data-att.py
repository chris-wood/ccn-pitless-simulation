...
def main(argv):
    delays = {}
    fileNames = ["data_OnData/delay-data-pitfull-cache-att",
                 "data_OnData/delay-data-pitless-cache-att",
                 "data_OnData/delay-data-pitless-cache-hybrid-att"]
    numberOfFiles = 3
    ...
    ax.set_ylabel(r"Content Forwarding Delay ($\mu$sec)")
    ax.grid(True)
    ax.legend(legendLabels, loc=1)

    print "Saving the plot..."
    pdf = PdfPages("delay-data-att.pdf")
    pdf.savefig(fig)
    pdf.close()
