...
def main(argv):
    delays = {}
    fileNames = ["data_OnInterest/delay-int-pitfull-cache-att",
                 "data_OnInterest/delay-int-pitless-cache-att",
                 "data_OnInterest/delay-int-pitless-cache-hybrid-att"]
    numberOfFiles = 3
    ...
    ax.set_ylabel(r"Interest Forwarding Delay ($\mu$sec)")
    ax.grid(True)
    ax.legend(legendLabels, loc=1)

    print "Saving the plot..."
    pdf = PdfPages("delay-int-att.pdf")
    pdf.savefig(fig)
    pdf.close()
